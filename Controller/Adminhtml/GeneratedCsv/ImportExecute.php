...
class ImportExecute extends Action
{
    ...
    public function execute(): Json
    {
        ...
        try {
            ...
            $this->logger->info('SquadexaAI ImportExecute: Import options: ' . print_r($importOptions, true));
            ...
            if ($customCsvFile && $customCsvFile['tmp_name']) {
                ...
                $this->logger->info('SquadexaAI ImportExecute: Custom CSV uploaded and saved as ' . $csvFilePath);
            } else {
                // Use default linked CSV
                $csvFileName = $generatedCsv->getResponseFileName();
                $csvFilePath = 'AIProductCreator/Output/' . $csvFileName;
                $this->logger->info('SquadexaAI ImportExecute: Using default linked CSV: ' . $csvFilePath);
            }
            // Validate CSV
            ...
            if (!$validationResult['is_valid']) {
                ...
                $errorReportUrl = $this->getUrl('squadkin_squadexaai/generatedcsv/downloaderrorreport', ['file' => $errorReportFile]);
                return $resultJson->setData([
                    'success' => false,
            ...
            if ($customCsvFile && $customCsvFile['tmp_name']) {
                $this->logger->info('SquadexaAI ImportExecute: Importing directly from custom CSV file: ' . $csvFilePath);
            ...
            $this->logger->info('SquadexaAI ImportExecute: Starting import service (AI product flow)');
            $result = $this->importService->importProductsFromGeneratedCsv($generatedCsv, $importOptions);
            $this->logger->info('SquadexaAI ImportExecute: Import completed successfully. Imported: ' . $result['imported_count'] . ' / ' . $result['total_count']);
            return $resultJson->setData([
                'success' => true,
            ...
            $this->logger->error('SquadexaAI ImportExecute: LocalizedException: ' . $e->getMessage());
            ...
            try {
                ...
                $this->logger->error('SquadexaAI ImportExecute: Failed to reset status: ' . $saveException->getMessage());
            }
