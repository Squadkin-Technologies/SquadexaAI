...
class TokenStatus extends Field
{
    ...
    protected function _getElementHtml(AbstractElement $element): string
    {
        ...
        $html = '<div class="token-status-info" ';
        $html .= 'style="padding: 15px; background: #f8f9fa; border-radius: 5px; margin: 10px 0;">';
        ...
        if (!empty($apiKey)) {
            try {
                ...
                if (isset($apiKeyMetadata['created_at'])) {
                    ...
                    $html .= '<div style="margin-bottom: 8px;"><strong>🕒 Created:</strong> ';
                    $html .= htmlspecialchars($formattedDate) . '</div>'; // phpcs:ignore
                }
                
                if (isset($apiKeyMetadata['last_used_at'])) {
                    $formattedDate = $this->formatDateTime($apiKeyMetadata['last_used_at']);
                    // phpcs:ignore Magento2.Functions.DiscouragedFunction
                    $html .= '<div style="margin-bottom: 8px;"><strong>🔄 Last Used:</strong> ';
                    $html .= htmlspecialchars($formattedDate) . '</div>'; // phpcs:ignore
                }
                
                $html .= '<div style="margin-bottom: 8px;"><strong>🔐 Status:</strong> ';
                $html .= '<span style="color: green;">Active (Never Expires)</span></div>';
                
                // Display usage statistics if available
                if (isset($apiKeyMetadata['usage_count'])) {
                    $html .= '<div style="margin-bottom: 8px;">';
                    $html .= '<strong>📊 Total API Calls:</strong> ';
                    $html .= number_format($apiKeyMetadata['usage_count']) . '</div>';
                }
                
                $html .= '<div style="color: #666; font-size: 12px; margin-top: 10px;">';
                $html .= '💡 This API key never expires and is used for all API requests.</div>';
                
            } catch (\Exception $e) {
                // If metadata fetch fails, show basic info
                $html .= '<div style="color: green; font-weight: bold; margin-bottom: 10px;">';
                $html .= '✅ API Key Configured</div>';
                $html .= '<div style="margin-bottom: 8px;"><strong>🔐 Status:</strong> ';
                $html .= '<span style="color: green;">Active (Never Expires)</span></div>';
                $html .= '<div style="margin-bottom: 8px;"><strong>🔑 Key Length:</strong> ';
                $html .= strlen($apiKey) . ' characters</div>';
                $html .= '<div style="color: #666; font-size: 12px; margin-top: 10px;">';
                $html .= '💡 This API key never expires and is used for all API requests.</div>';
            }
            ...
            $html .= '<div style="margin-top: 10px; padding: 15px; ';
            $html .= 'background: #e7f3ff; border-left: 4px solid #2196F3; border-radius: 4px;">';
            $html .= '<strong style="color: #1976D2;">🚀 Quick Setup Guide:</strong>';
            $html .= '<ol style="margin: 10px 0 0 20px; color: #333;">';
            $html .= '<li style="margin-bottom: 8px;">Enter your ';
            $html .= '<strong>Squadexa AI email</strong> and <strong>password</strong> ';
            $html .= 'in the fields above</li>';
            $html .= '<li style="margin-bottom: 8px;">Click the <strong>"Generate API Key"</strong> button</li>';
            $html .= '<li style="margin-bottom: 8px;">Your permanent API key will be ';
            $html .= '<strong>generated automatically</strong></li>';
            $html .= '<li style="margin-bottom: 8px;">';
            $html .= 'Start generating AI-powered product descriptions immediately!</li>';
            $html .= '</ol>';
            $html .= '<div style="margin-top: 10px; padding: 8px; ';
            $html .= 'background: #fff9e6; border-radius: 3px;">';
            $html .= '<small>💡 <strong>Don\'t have an account?</strong> ';
            $html .= 'Visit <a href="https://squadexa.ai/" target="_blank" ';
            $html .= 'style="color: #2196F3;">Squadexa AI Portal</a> to register first.</small>';
            $html .= '</div>';
            $html .= '</div>';
        }
